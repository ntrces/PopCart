...
$update_all_stmt = $conn->prepare("UPDATE shipping_address SET status = 'others' WHERE user_id = ?");
$update_all_stmt->bind_param("i", $user_id);
$update_all_stmt->execute();
$update_all_stmt->close();

// Set the specific address to 'default'
$update_stmt = $conn->prepare("UPDATE shipping_address SET status = 'default' WHERE shipping_address_id = ? AND user_id = ?");
