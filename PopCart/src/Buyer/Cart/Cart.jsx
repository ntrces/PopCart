...
<button className="sign-out" onClick={() => setShowSignOutModal(true)}>
  ...
  <div className="modal-overlay-cart">
    <div className="signout-modal-cart">
      <h3>Sign Out</h3>
      <p>Are you sure you want to sign out?</p>

      <div className="modal-buttons-cart">
        <button className="cancel-btn-cart" onClick={() => setShowSignOutModal(false)}>
          Cancel
        </button>

        <button className="confirm-btn-cart" onClick={() => { localStorage.removeItem('user'); setShowSignOutModal(false); navigate('/signin'); }}>
          Sign Out
        </button>
      </div>
    </div>
