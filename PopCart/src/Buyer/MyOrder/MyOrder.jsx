  ...
  useEffect(() => {
    if (user) {
      const fetchOrders = async () => {
        try {
          const response = await fetch(apiUrl(`get_orders.php?user_id=${user.user_id}`));
          const data = await response.json();
          if (data.success) {
            // For each order, fetch its items (album_title, artist, qty)
            const ordersWithItems = await Promise.all(data.orders.map(async (order) => {
              try {
                const r = await fetch(apiUrl(`get_order_products.php?order_header_id=${order.order_header_id}`));
...
<path d="M15 15.8333C16.3807 15.8333 17.5 14.7141 17.5 13.3333C17.5 11.9526 16.3807 10.8333 15 10.8333C13.6193 10.8333 12.5 11.9526 12.5 13.3333C12.5 14.7141 13.6193 15.8333 15 15.8333Z" stroke="#8B5CF6" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <div className="right-controls">
         ...
         <Link to="/buyer/Buyernotif" className="icon-btn">
...
<button className="sign-out" onClick={() => setShowSignOutModal(true)}>
        ...
        {showSignOutModal && (
  <div className="modal-overlay-MyO">
    <div className="signout-modal-MyO">
      <h3>Sign Out</h3>
      <p>Are you sure you want to sign out?</p>

      <div className="modal-buttons-MyO">
        <button className="cancel-btn-MyO" onClick={() => setShowSignOutModal(false)}>
          Cancel
        </button>

        <button className="confirm-btn-MyO" onClick={() => { localStorage.removeItem('user'); setShowSignOutModal(false); navigate('/signin'); }}>
          Sign Out
        </button>
      </div>
    </div>
